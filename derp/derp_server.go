...
func (s *Server) RemovePacketForwarder(dst key.NodePublic, fwd PacketForwarder) {
	...
	if m, ok := v.(*multiForwarder); ok {
		...
		if remain, isLast := m.delete(fwd); isLast {
...
// delete removes a packet forwarder from the map. If only one forwarder remains after
// the removal, it will be returned alongside a `true` boolean value.
func (f *multiForwarder) delete(fwd PacketForwarder) (_ PacketForwarder, isLast bool) {
	delete(f.all, fwd)
